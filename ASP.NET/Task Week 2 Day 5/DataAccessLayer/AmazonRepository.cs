    ...
    public class AmazonRepository : IAmazonRepository
    {
        ...
        public async Task<List<Orderd>> GetAllOrdersByCreatedDate(DateTime n)
        {
            using (AmazonDbContext dbContext = new AmazonDbContext())
            {
                ...
                var countries2 = await dbContext.Orders.Where(x => x.CreatedDate > n).ToListAsync();
        ...
        public async Task<List<string>> GetAllUserNames()
        {
            using (AmazonDbContext dbContext = new AmazonDbContext())
            {
                ...
                List<string> orders = await dbContext.Orders.Select(x => x.Name).ToListAsync();
        ...
        public async Task<List<Orderd>> GetAllOrdersByCountryName(List<string> order)
        {
            using (AmazonDbContext dbContext = new AmazonDbContext())
            {
...

                {
                    ...
                    var countries2 = await dbContext.Orders.Include(x => x.Amazon).Where(x => x.Amazon.Name == item).ToListAsync();
        ...
        public async Task<List<Ordercostgroup>> GetSumOfOrdersByCountry()
        {
            ...
            var sumgrp = await dbContext.Orders.GroupBy(x => x.Amazon.Id).Select(x =>
            new Ordercostgroup
            {
